...
这个数据集当前一共有 `1,600` 条样本, 一共 `4` 个标签:

| 标签 | 含义(可粗略理解为) | 样本数 |
| :---: | :---: | :---: |
| `还款` | 账单、还款、应还金额等问题 | `688` |
| `收支分析` | 消费统计、收入支出、流水分析 | `263` |
| `申卡` | 信用卡申请与办卡相关问题 | `185` |
| `优惠（聚优惠）` | 优惠活动、权益、折扣相关问题 | `464` |
...
1. 样本整体不长;
2. 大多数句子不需要复杂的长文本切块;
3. 用 `max_length=80` 作为课堂默认值会更稳妥, 已经能覆盖 `99%+` 的样本;
4. 如果想完全覆盖这个数据集, 可以设成 `96`.
...
如果学员使用的是 macOS 并且机器较新, `torch` 还可以使用 Apple 的 `MPS` 后端做加速; 如果没有可用加速设备, 用 CPU 也能完成这个 1,600 条样本的小型教学任务, 只是会慢一些.
...
当 `loss` 算出来以后, 模型还不知道应该改哪里. 它只知道:“我这次猜错了, 但到底是哪个参数该增大, 哪个参数该减小?” 这时就要靠反向传播.

从数学上看, 反向传播做的是:

> **对每一个可训练参数, 计算它对最终损失的偏导数.**

这个偏导数就是梯度. 在代码里, 我们通常并不手写这些矩阵求导, 而是交给 PyTorch 的 `autograd` 系统完成.
...
对于这份只有 `1,600` 条样本的意图识别数据, 一开始可以用比较稳妥的配置:
